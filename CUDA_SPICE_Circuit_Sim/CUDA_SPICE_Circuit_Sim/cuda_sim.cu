...
void full_CudaTran(Netlist* net, CUDA_Net* dev_net, float start, float stop, float step) {
	...
	float* vSweepMat = mat1D(n_nodes + 1);
	float** vOut = (float**)malloc((n_steps - skipped_steps) * sizeof(float*));
	...
	char** names = net->netNames.data();
	names[0] = "Time(s)";
	mat2DtoCSV(names, vOut, n_steps - skipped_steps, n_nodes + 1, "transient.csv");
	names[0] = "gnd";
	...
	cudaFree(dev_vGuess);
	cudaFree(dev_vPrev);
	freeMat2D(vOut, n_steps - skipped_steps);
}